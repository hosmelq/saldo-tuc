...
public class CardUpdateActivity extends BaseActivity {
    ...
    private void bindCard(Card card) {
        mCardNameInput.setText(card.getName());
        mCardNumberInput.setText(card.getNumber());

        if (card.getNotifications()) {
            mCardSubscribe.setChecked(true);
        }
    }
    ...
    private void storeCard() {
        ...
        Card.getQuery()
            ...
            .getFirstInBackground((object, e) -> {
                if (e == null && !number.equals(mCard.getNumber())) {
                    new AlertDialog.Builder(CardUpdateActivity.this)
                ...
                String oldNumber = mCard.getNumber();
                mCard.setName(name);
                mCard.setNumber(number);

                if (!number.equals(oldNumber)) {
                    mCard.setBalance("");
                    ParsePush.unsubscribeInBackground(Config.PARSE_CARD_CHANNEL_PREFIX + oldNumber);
                }

                if (mCardSubscribe.isChecked()) {
                    if (!mCard.getNotifications() && !SyncHelper.isOnline(CardUpdateActivity.this)) {
                        intent.putExtra(CardsActivity.WILL_SUBSCRIBE_WHEN_ONLINE, true);
                    }

                    mCard.setNotifications(true);
                    ParsePush.subscribeInBackground(mCard.getChannelName());
                } else {
                    if (mCard.getNotifications() && !SyncHelper.isOnline(CardUpdateActivity.this)) {
                        intent.putExtra(CardsActivity.WILL_UNSUBSCRIBE_WHEN_ONLINE, true);
                    }

                    mCard.setNotifications(false);
                }

                mCard.pinInBackground(e1 -> {
                    if (e1 == null) {
